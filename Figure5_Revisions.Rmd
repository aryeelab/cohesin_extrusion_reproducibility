...
freq <- left_plus %>% 
  ...
  dplyr::mutate(perc_conv = 100*(num_convergent / num_fragments)) %>% 
...
left_plus %>% 
  ...
  mutate(convergent = as.factor(convergent)) %>% 
  ggplot(aes(x = FL1, fill = convergent, group = convergent, color = convergent))+
...
```{r}
left_plus %>% 
  mutate(FL2 = end2-start2) %>% 
  ggplot(aes(x = FL2))+
  geom_histogram()+
  facet_wrap(~convergent,ncol=1,
             scales = "free")
```


# 5B
Figure 5B is a schematic illustrating how we annotated CBS with chromatin annotations. I made it in Keynote, attached below for reference:
